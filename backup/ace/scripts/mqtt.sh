...
setmqaut -m $1 -t topic -n SYSTEM.BASE.TOPIC          -p hgj -all +pub +sub
setmqaut -m $1 -t q     -n SYSTEM.MQTT.TRANSMIT.QUEUE -p hgj -all +put

cat /opt/mqm/mqxr/samples/installMQXRService_unix.mqsc | runmqsc $1

echo "START SERVICE(SYSTEM.MQXR.SERVICE)" | runmqsc $1

echo "DEFINE CHANNEL(MQTT) CHLTYPE(MQTT) PORT($2) MCAUSER('hgj')"  | runmqsc $1
